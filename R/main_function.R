  ...
  if(length(Samples)>0){
    ...
    if(length(unique(dt.infer.strandness.res$Strandness))==1 & unique(dt.infer.strandness.res$Strandness)!="unstranded"){
      ...
      if(nrow(dt.method.combo.selected.nonexomepeak2)>0){
        ...
        parallel.Method.except.exomePeak2.cmd <- paste0(bin.dir,"/parallel -j ", n.cores," --will-cite -a ", paste0(tmp.dir,"/parameter.Method.except.exomePeak2.parallel.txt") ," --colsep '\t' '",
